...
struct llama_hparams {
    ...
    // DSpark (V4.1) proposal blocks attend bidirectionally within the block and to one fixed
    // window of the last n_swa committed positions, the same for every block row
    // (reference get_dspark_topk_idxs). Other drafts keep the causal, per-row sliding mask.
