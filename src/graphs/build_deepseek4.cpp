...
static ggml_tensor * dsv4_build_lid_top_k(
    ...
    // V4 scores against this layer's own index keys under the shared LID plan; V4.1 against the
    // key owner's cache under the plan of whichever stream this layer reads
...
static void ds4_build_comp(ggml_tensor * cur, llm_build_context & llm, ggml_context * ctx0,
    ...
    // At ratio 1 there is nothing to pool and the file carries no gate (V4.1): the softmax then
    // runs over a single element and returns 1 whatever the score holds.
    ...
    if (comp_ape) {
        // V4 adds a learned position term inside the pooled group; V4.1 has none
...
// DeepSeek-V4.1 engram: gather 48 hashed rows per token, project them into one key per
// hyper-connection stream plus a shared value, gate the value with the normalized
// key.query dot product, and add it into every stream. Row indices come from the host
// (llama_set_engram_rows); the table itself is only ever touched through get_rows, so the
// 40+ GB tensor stays in the file mapping and is paged in a few rows at a time.
static ggml_tensor * ds4_build_engram(ggml_context * ctx0, llm_build_context & llm, ggml_tensor * x, int il, int eg) {
    ...
    // normalized per (token, hc copy) over n_embd; q scales the stream side, k the key side
...
static ggml_tensor * ds4_attention(ggml_cgraph * gf, ggml_context * ctx0, llm_build_context & llm, ggml_tensor * inpL,
    ...
    if (shared && slot_ready && hparams.dsv41_is_kv_source(il)) {
        // V4.1: only a source pools. Its rows land in the slot cache that every reader of this
        // stream is aliased onto, and the key owner writes its index keys from the same latent.
        ggml_tensor * pre_rope = nullptr;
    ...
    if (shared && (is_a || is_b) &&
        ...
        // V4.1: the index source of this stream picks the rows, and the layers after it in the
        // same stream reuse that pick. (Mainline instead lets those layers see every compressed
        // row; sharing is what the model was trained with.)
